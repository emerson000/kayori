    ...
    return (
        <div>
            ...
            <div className="clear-both">
                {pageNumber > 1 && <Link className="btn float-left" href={`?page=${pageNumber - 1}${search != "" ? `&search=${search}` : ''}`}>Previous</Link>}
                <Link className="btn float-right" href={`?page=${pageNumber + 1}${search != "" ? `&search=${search}` : ''}`}>Next</Link>
            </div>
